...
class TestLevenshteinPerformance:
    ...
    def test_levenshtein_small_objects(self):
        ...
        assert avg_time < 100.0, f"Levenshtein too slow: {avg_time:.2f}ms per call"
    ...
    def test_levenshtein_medium_objects(self):
        ...
        assert elapsed_ms < 2000.0, f"Levenshtein too slow: {elapsed_ms:.2f}ms"
        assert distance >= 0
    ...
    def test_levenshtein_worst_case(self):
        ...
        assert elapsed < 1.0, f"Levenshtein timeout: {elapsed:.2f}s"
...
class TestSimHashPerformance:
    ...
    def test_simhash_computation_speed(self):
        ...
        # Should be very fast
        assert avg_us < 1000, f"SimHash too slow: {avg_us:.0f}µs per call"
        assert hash_val >= 0
    ...
    def test_hamming_distance_speed(self):
        ...
        assert avg_ns < 10000, f"Hamming distance too slow: {avg_ns:.0f}ns"
...
class TestFastSimilarityMatcher:
    ...
    def test_filtering_with_50_objects(self):
        ...
        assert elapsed < 10.0, f"Matcher too slow: {elapsed:.1f}s for 10 objects"
    ...
    def test_filtering_with_100_objects(self):
        ...
        assert elapsed < 60.0, f"Matcher timeout: {elapsed:.1f}s for 20 objects."
